...
-- pool_type: 1私有池 2公共池 3美团专属池 4饿了么专属池 5抖音专属池
-- ---------------------------------------------------------------------------
...
-- can_use_after_draw: 0暂不可用 1可用
-- source_pool_type: 1私有池 2公共池 3美团专属 4饿了么专属 5抖音专属
...
-- ---------------------------------------------------------------------------
-- 6.8 第三方团购登记记录表（美团/饿了么/抖音）
-- channel: 1美团 2饿了么 3抖音；qr_code_unique_key 识别二维码渠道
-- ---------------------------------------------------------------------------
...
-- YBG-C-RETURN-001 幂等迁移：团购登记折算赠送余额（MySQL 8 无 ADD COLUMN IF NOT EXISTS，用存储过程）
DROP PROCEDURE IF EXISTS ybg_return_add_cols;
DELIMITER //
CREATE PROCEDURE ybg_return_add_cols()
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='third_group_verify_record' AND column_name='draw_batch_no') THEN
    ALTER TABLE third_group_verify_record ADD COLUMN draw_batch_no VARCHAR(64) DEFAULT NULL COMMENT '抽奖批次号(折算防重)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='third_group_verify_record' AND column_name='return_balance') THEN
    ALTER TABLE third_group_verify_record ADD COLUMN return_balance DECIMAL(10,2) DEFAULT 0.00 COMMENT '登记折算赠送余额';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='third_group_verify_record' AND column_name='return_desc') THEN
