 ...
 * ============================================================ */
(function(){
  var BASE = (window.IBIGOU_API_BASE) || '';
  ...
  async function formRequest(method, path, params){
    var url = fullUrl(path);
    ...
    try {
      var resp = await fetch(url, { method: method, headers: headers, body: body.toString() });
      var ct = resp.headers.get('content-type') || '';
      var data;
      if (ct.indexOf('application/json') >= 0) data = await resp.json();
      else data = { _raw: await resp.text() };
      return data;
    } catch (e) {
      console.warn('[api] net error', method, path, e.message);
      return { code: 599, msg: 'network error: ' + e.message };
    }
  }
  ...
  var api = {
    ...
    customer: {
      ...
      offlineCalc: function(body){ return formRequest('POST', '/api/customer/offline/calc', body); },
      offlineOrder: function(body){ return formRequest('POST', '/api/customer/offline/order', body); },
    ...
    // 商家
    merchant: {
      login: function(body){ var p = typeof body === 'string' ? { account: body, password: arguments[1] || '' } : body; return formRequest('POST', '/api/merchant/auth/login', p); },
      ...
      prizePools: function(){ return request('GET', '/api/merchant/config/prize-pools'); },
      savePrizePool: function(body){ return formRequest('POST', '/api/merchant/config/prize-pools', body); },
      deletePrizePool: function(id){ return request('DELETE', '/api/merchant/config/prize-pools/' + encodeURIComponent(id)); },
  ...
  memberDetail: function(phone){ return request('GET', '/api/merchant/members/' + encodeURIComponent(phone) + '/detail'); },
  saveProfile: function(phone, body){ return request('POST', '/api/merchant/members/' + encodeURIComponent(phone) + '/profile', body); },
  addVisit: function(phone, body){ return request('POST', '/api/merchant/members/' + encodeURIComponent(phone) + '/visit', body); },
  ...
  upcomingAppointments: function(){ return request('GET', '/api/merchant/appointments/upcoming'); },
      ...
      memberTypes: function(){ return request('GET', '/api/merchant/member-types'); },
      addMemberType: function(body){ return request('POST', '/api/merchant/member-types', body); },
      deleteMemberType: function(id){ return request('DELETE', '/api/merchant/member-types/' + id); },
      ...
      couponVerifyManual: function(couponId){ return formRequest('POST', '/api/merchant/coupon/verify-manual', { couponId: couponId }); },
      balanceManualDeduct: function(phone, amt){ return formRequest('POST', '/api/merchant/balance/manual-deduct', { userPhone: phone, orderAmount: amt }); },
      ...
      announcePublish: function(content){ return formRequest('POST', '/api/merchant/announce/publish', { content: content }); },
      ...
      messagesReadAll: function(){ return formRequest('POST', '/api/merchant/messages/read-all', {}); },
