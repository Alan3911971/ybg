...
(function(){
  ...
  var _placeholder_unused = document.getElementById("channelChip");
  if (channelChip) channelChip.textContent = chName;
  ...
  async function pickPrize(){
    ...
    if (api && ph) {
      var r = await api.customer.drawNormal(ph, 'M1001');
  ...
  function tick(now){
    ...
    var remain = Math.max(0, 15000 - elapsed);
