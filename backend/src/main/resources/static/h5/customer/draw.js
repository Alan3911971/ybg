...
(function(){
  ...
  if (!tk) { location.replace("/h5/customer/login.html?redirect=" + encodeURIComponent("draw.html" + location.search)); return; }
  ...
  var MERCHANT_NO = (function(){ try {
    var m = location.search.match(/merchantNo=([^&]+)/);
    if (m) { localStorage.setItem('ibigou_merchant_no', m[1]); return m[1]; }
    var stored = localStorage.getItem('ibigou_merchant_no');
    if (stored === 'M001') { localStorage.removeItem('ibigou_merchant_no'); return ''; }
    return stored || '';
  } catch(e){ return ''; } })();
  ...
  function buildPayUrl(p){
    ...
    url += "&merchantNo=" + encodeURIComponent(MERCHANT_NO);
    return url;
  }
})();